...
const API_KEY = "rO9LK8gs.GJlDmsesHDEAIepvjMblAx8s1nETM60O";
...
const plan_id = 'plan_c1e072c447d042d6b02d7431d8f8b485';
...
describe("Testing Customers Endpoints", () => {
    ...
    it("Test Get Customer Details", async () => {
        ...
        const customer = result.data?.length ? result.data[0] : null;
        if (customer) {
            const customerDetails = await lotus.getCustomer({customerId:customer.customer_id});
            expect(customerDetails.status).toEqual(200);
            const hasAllKeys = expectedCustomerKeys.every((item) => customerDetails.data.hasOwnProperty(item));
            expect(hasAllKeys).toEqual(true);
        }
    });
